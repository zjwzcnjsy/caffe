...
#include "caffe/layers/angular_triplet_loss_layer.hpp"

namespace caffe
{

template <typename Dtype>
void AngularTripletLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype> *> &bottom,
                                                const vector<Blob<Dtype> *> &top)
{
  ...
  type_ = this->layer_param_.angular_triplet_loss_param().type();
  iter_ = this->layer_param_.angular_triplet_loss_param().iteration();
  lambda_ = (Dtype)0.;

  const int num_output = this->layer_param_.angular_triplet_loss_param().num_output();
  N_ = num_output;
  const int axis = bottom[0]->CanonicalAxisIndex(
      this->layer_param_.angular_triplet_loss_param().axis());
...
template <typename Dtype>
void AngularTripletLossLayer<Dtype>::Reshape(const vector<Blob<Dtype> *> &bottom,
                                             const vector<Blob<Dtype> *> &top)
{
  ...
  switch (type_)
  {
  case AngularTripletLossParameter_MarginType_SINGLE:
    break;
  case AngularTripletLossParameter_MarginType_DOUBLE:
    cos_theta_quadratic_.Reshape(top_shape);
    break;
  case AngularTripletLossParameter_MarginType_TRIPLE:
    cos_theta_quadratic_.Reshape(top_shape);
    cos_theta_cubic_.Reshape(top_shape);
    sign_1_.Reshape(top_shape);
    sign_2_.Reshape(top_shape);
    break;
  case AngularTripletLossParameter_MarginType_QUADRUPLE:
    cos_theta_quadratic_.Reshape(top_shape);
    cos_theta_cubic_.Reshape(top_shape);
...
template <typename Dtype>
void AngularTripletLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom,
                                                 const vector<Blob<Dtype> *> &top)
{
  iter_ += (Dtype)1.;
  Dtype base_ = this->layer_param_.angular_triplet_loss_param().base();
  Dtype gamma_ = this->layer_param_.angular_triplet_loss_param().gamma();
  Dtype power_ = this->layer_param_.angular_triplet_loss_param().power();
  Dtype lambda_min_ = this->layer_param_.angular_triplet_loss_param().lambda_min();
  ...
  case AngularTripletLossParameter_MarginType_DOUBLE:
  {
    ...
    for (int i = 0; i < M_; i++)
    {
      ...
      top_data[i * N_ + label_value] = x_norm_data[i] * ((Dtype)2. * sign_0_data[i * N_ + label_value] *
                                                             cos_theta_quadratic_data[i * N_ + label_value] -
                                                         (Dtype)1.);
    }
    // + lambda * x'w
...
template <typename Dtype>
void AngularTripletLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype> *> &top,
                                                  ...
                                                  const vector<Blob<Dtype> *> &bottom)
{
      ...
      for (int i = 0; i < M_; i++)
      {
        ...
        for (int j = 0; j < N_; j++)
        {
          ...
          else
          {
            ...
            Dtype coeff_x = (Dtype)1. / (-x_norm_data[i]) * ((Dtype)2. * sign_0_data[i * N_ + j] * cos_theta_quadratic_data[i * N_ + j] + (Dtype)1.);
      ...
      for (int i = 0; i < M_; i++)
      {
        ...
        for (int j = 0; j < N_; j++)
        {
          ...
          else
          {
            ...
            Dtype coeff_x = (Dtype)1. / (-x_norm_data[i]) * ((Dtype)8. * sign_1_data[i * N_ + j] * cos_theta_cubic_data[i * N_ + j] - sign_2_data[i * N_ + j]);
            Dtype coeff_norm = sqrt(coeff_w * coeff_w + coeff_x * coeff_x);
            coeff_w = coeff_w / coeff_norm;
            coeff_x = coeff_x / coeff_norm;
            caffe_cpu_axpby(K_, (Dtype)1. / ((Dtype)1. + lambda_) * top_diff[i * N_ + j] * coeff_w,
      ...
      for (int i = 0; i < M_; i++)
      {
        ...
        for (int j = 0; j < N_; j++)
        {
          ...
          else
          {
            ...
            //                        sign_4) * x
            Dtype coeff_x = (Dtype)1. / (-x_norm_data[i]) * (sign_3_data[i * N_ + j] * ((Dtype)24. * cos_theta_quartic_data[i * N_ + j] - (Dtype)8. * cos_theta_quadratic_data[i * N_ + j] - (Dtype)1.) - sign_4_data[i * N_ + j]);
            Dtype coeff_norm = sqrt(coeff_w * coeff_w + coeff_x * coeff_x);
            coeff_w = coeff_w / coeff_norm;
            coeff_x = coeff_x / coeff_norm;
            caffe_cpu_axpby(K_, (Dtype)1. / ((Dtype)1. + lambda_) * top_diff[i * N_ + j] * coeff_w,
  ...
  Dtype *top_data = top[0]->mutable_cpu_data();
  const bool triplet_flag = this->layer_param_.angular_triplet_loss_param().triplet();
  const bool semihard_flag = this->layer_param_.angular_triplet_loss_param().semihard();
  const bool hard_flag = (type_ != AngularTripletLossParameter_MarginType_SINGLE);
...
#ifdef CPU_ONLY
STUB_GPU(AngularTripletLossLayer);
#endif

INSTANTIATE_CLASS(AngularTripletLossLayer);
REGISTER_LAYER_CLASS(AngularTripletLoss);
