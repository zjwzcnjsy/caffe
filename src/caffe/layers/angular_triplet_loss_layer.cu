...
{
  // top_data为加了间隔的分类层的输出
  // margin_top_data为softmax分类层的输出
  CUDA_KERNEL_LOOP(index, nthreads)
  {
    ...
    if (label_i == argmax1) // easy example，也就是加了间隔的分类层也分对了，那么这个应该大概率是easy example
    {
      for (int k = 0; k < K_; k++)
      {
    ...
    else if (label_i != argmax2 && hard_flag) // hard example，2个分类层都分错了，这个样本可能是噪声。
    {
      for (int k = 0; k < K_; k++)
      {
        bottom_diff[index * K_ + k] = Dtype(0);
  ...
  CUDA_KERNEL_LOOP(index, nthreads)
  {
    ...
    if (label_value == j)
    {
      ...
      top[index] += x_norm[i] * (sign_3[index] * ((Dtype)8. * cos_theta_quartic[index] -
                                                  (Dtype)8. * cos_theta_quadratic[index] + (Dtype)1.) +
                                 sign_4[index]);
      top[index] /= ((Dtype)1. + lambda);
    }
  }
    ...
    for (int n = 0; n < N_; n++)
    {
      ...
      else
      {
        if (x_norm[i] > 1e-3)
        {
          ...
          Dtype coeff_x = -(Dtype)1. / x_norm[i] * ((Dtype)2. * sign_0[i * N_ + n] * cos_theta_quadratic[i * N_ + n] + (Dtype)1.);
  ...
  CUDA_KERNEL_LOOP(index, nthreads)
  {
    ...
    for (int n = 0; n < N_; n++)
    {
      ...
      else
      {
        ...
        Dtype coeff_x = -(Dtype)1. / x_norm[i] * ((Dtype)8. * sign_1[i * N_ + n] * cos_theta_cubic[i * N_ + n] - sign_2[i * N_ + n]);
        Dtype coeff_norm = sqrt(coeff_w * coeff_w + coeff_x * coeff_x);
        coeff_w = coeff_w / coeff_norm;
        coeff_x = coeff_x / coeff_norm;
        bottom_diff[index] += (Dtype)1. / ((Dtype)1. + lambda) * top_diff[i * N_ + n] *
  ...
  CUDA_KERNEL_LOOP(index, nthreads)
  {
    ...
    for (int n = 0; n < N_; n++)
    {
      ...
      else
      {
        ...
        Dtype coeff_x = -(Dtype)1. / x_norm[i] * (sign_3[i * N_ + n] * ((Dtype)24. * cos_theta_quartic[i * N_ + n] - (Dtype)8. * cos_theta_quadratic[i * N_ + n] - 1) - sign_4[i * N_ + n]);
        Dtype coeff_norm = sqrt(coeff_w * coeff_w + coeff_x * coeff_x);
        coeff_w = coeff_w / coeff_norm;
        coeff_x = coeff_x / coeff_norm;
        bottom_diff[index] += (Dtype)1. / ((Dtype)1. + lambda) * top_diff[i * N_ + n] *
...
template <typename Dtype>
void AngularTripletLossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype> *> &bottom,
...
template <typename Dtype>
void AngularTripletLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype> *> &top,
...
INSTANTIATE_LAYER_GPU_FUNCS(AngularTripletLossLayer);
