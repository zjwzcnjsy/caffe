...
cv::Mat FaceAlignDataLayer<Dtype>::bestFitRect(const cv::Mat& groundTruth, const cv::Mat& meanShape)
{
  ...
  for (int i = 0; i < groundTruth.rows; ++i) {
    ...
    if (x < box_pt1.x) { box_pt1.x = x; }
    if (y < box_pt1.y) { box_pt1.y = y; }
    if (x > box_pt2.x) { box_pt2.x = x; }
    if (y > box_pt2.y) { box_pt2.y = y; }
    x = meanShape.at<float>(i, 0);
    y = meanShape.at<float>(i, 1);
    if (x < box2_pt1.x) { box2_pt1.x = x; }
    if (y < box2_pt1.y) { box2_pt1.y = y; }
    if (x > box2_pt2.x) { box2_pt2.x = x; }
    if (y > box2_pt2.y) { box2_pt2.y = y; }
  }
  ...
  for (int i = 0; i < S0.rows; ++i) {
    ...
    if (x < box3_pt1.x) { box3_pt1.x = x; }
    if (y < box3_pt1.y) { box3_pt1.y = y; }
    if (x > box3_pt2.x) { box3_pt2.x = x; }
    if (y > box3_pt2.y) { box3_pt2.y = y; }
  }
...
cv::Mat FaceAlignDataLayer<Dtype>::generatePerturbation(const cv::Mat& groundTruth, 
  ...
  cv::Rect_<float> scaledMeanRect;
  boundingRect(scaledMeanShape, scaledMeanRect);

  float rotationStdDevRad = rotationStdDev_ * CV_PI / 180;
  float translationXStdDev = translationMultX_ * scaledMeanRect.width;
  float translationYStdDev = translationMultY_ * scaledMeanRect.height;
...
template <typename Dtype>
void FaceAlignDataLayer<Dtype>::cropResizeRotate(
      const cv::Mat& meanShape,
      const cv::Mat& image, 
      const cv::Mat& initLandmark, 
