...
#define sign(x) ((x)>=0?1:-1)
	...
	template <typename Dtype>
	void caffe_bin_conv(const Blob<Dtype>* in, BinaryConvolutionParameter* conv_param,
	...
	template void caffe_bin_conv(const Blob<float>* in,
		BinaryConvolutionParameter* conv_param,
	...
	class BinaryConvolutionV2LayerTest : public MultiDeviceTest<TypeParam> {
		typedef typename TypeParam::Dtype Dtype;

	protected:
		BinaryConvolutionV2LayerTest()
			: blob_bottom_(new Blob<Dtype>(2, 32, 64, 64)),
			blob_bottom_2_(new Blob<Dtype>(2, 32, 64, 64)),
		...
		virtual void SetUp() {
			...
			caffe_cpu_sign(this->blob_bottom_2_->count(), this->blob_bottom_2_->cpu_data(), this->blob_bottom_2_->mutable_cpu_data());
			blob_bottom_vec_.push_back(blob_bottom_);
			blob_top_vec_.push_back(blob_top_);
		}

		virtual ~BinaryConvolutionV2LayerTest() {
			delete blob_bottom_;
	...
	TYPED_TEST_CASE(BinaryConvolutionV2LayerTest, TestDtypesAndDevices);

	TYPED_TEST(BinaryConvolutionV2LayerTest, TestSetup) {
		typedef typename TypeParam::Dtype Dtype;
		...
		EXPECT_EQ(this->blob_top_->height(), 31);
		EXPECT_EQ(this->blob_top_->width(), 31);
		EXPECT_EQ(this->blob_top_2_->num(), 2);
		EXPECT_EQ(this->blob_top_2_->channels(), 4);
		EXPECT_EQ(this->blob_top_2_->height(), 31);
		EXPECT_EQ(this->blob_top_2_->width(), 31);
		// setting group should not change the shape
		convolution_param->set_num_output(4);
		convolution_param->set_group(2);
		layer.reset(new BinaryConvolutionV2Layer<Dtype>(layer_param));
		layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
		EXPECT_EQ(this->blob_top_->num(), 2);
		EXPECT_EQ(this->blob_top_->channels(), 4);
		EXPECT_EQ(this->blob_top_->height(), 31);
	...
	TYPED_TEST(BinaryConvolutionV2LayerTest, TestSimpleBinaryConvolutionWithTestPhase) {
		typedef typename TypeParam::Dtype Dtype;
		this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
		this->blob_top_vec_.push_back(this->blob_top_2_);
		LayerParameter layer_param;
	...
	TYPED_TEST(BinaryConvolutionV2LayerTest, TestSimpleBinaryConvolutionWithTrainPhase) {
		typedef typename TypeParam::Dtype Dtype;
		this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
		this->blob_top_vec_.push_back(this->blob_top_2_);
		LayerParameter layer_param;
	...
	//TYPED_TEST(BinaryConvolutionV2LayerTest, TestGradient) {
	//  typedef typename TypeParam::Dtype Dtype;
	//  LayerParameter layer_param;
	//  BinaryConvolutionParameter* convolution_param =
	//      layer_param.mutable_binary_convolution_param();
	...
	//	BinaryConvolutionV2Layer<Dtype> layer(layer_param);
